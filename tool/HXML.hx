// Source: https://github.com/openfl/hxp/
package;
...
abstract HXML(Array<String>)
{
	...
	**/
	...
	public function new(hxml:HXMLConfig = null)
	{
		...
		if (hxml != null)
		{
			if (Reflect.hasField(hxml, "as3"))
				as3 = hxml.as3;
			if (Reflect.hasField(hxml, "connect"))
				connect = hxml.connect;
			if (Reflect.hasField(hxml, "cpp"))
				cpp = hxml.cpp;
			if (Reflect.hasField(hxml, "cppia"))
				cppia = hxml.cppia;
			if (Reflect.hasField(hxml, "cs"))
				cs = hxml.cs;
			if (Reflect.hasField(hxml, "dce"))
				dce = hxml.dce;
			if (Reflect.hasField(hxml, "debug"))
				debug = hxml.debug;
			if (Reflect.hasField(hxml, "display"))
				display = hxml.display;
			if (Reflect.hasField(hxml, "flashStrict"))
				flashStrict = hxml.flashStrict;
			if (Reflect.hasField(hxml, "genHXClasses"))
				genHXClasses = hxml.genHXClasses;
			if (Reflect.hasField(hxml, "hl"))
				hl = hxml.hl;
			if (Reflect.hasField(hxml, "java"))
				java = hxml.java;
			if (Reflect.hasField(hxml, "js"))
				js = hxml.js;
			if (Reflect.hasField(hxml, "lua"))
				lua = hxml.lua;
			if (Reflect.hasField(hxml, "main"))
				main = hxml.main;
			if (Reflect.hasField(hxml, "neko"))
				neko = hxml.neko;
			if (Reflect.hasField(hxml, "noInline"))
				noInline = hxml.noInline;
			if (Reflect.hasField(hxml, "noOpt"))
				noOpt = hxml.noOpt;
			if (Reflect.hasField(hxml, "noOutput"))
				noOutput = hxml.noOutput;
			if (Reflect.hasField(hxml, "noTraces"))
				noTraces = hxml.noTraces;
			if (Reflect.hasField(hxml, "php"))
				php = hxml.php;
			if (Reflect.hasField(hxml, "phpFront"))
				phpFront = hxml.phpFront;
			if (Reflect.hasField(hxml, "phpLib"))
				phpLib = hxml.phpLib;
			if (Reflect.hasField(hxml, "phpPrefix"))
				phpPrefix = hxml.phpPrefix;
			if (Reflect.hasField(hxml, "prompt"))
				prompt = hxml.prompt;
			if (Reflect.hasField(hxml, "python"))
				python = hxml.python;
			if (Reflect.hasField(hxml, "swf"))
				swf = hxml.swf;
			if (Reflect.hasField(hxml, "swfHeader"))
				swfHeader = hxml.swfHeader;
			if (Reflect.hasField(hxml, "swfVersion"))
				swfVersion = hxml.swfVersion;
			if (Reflect.hasField(hxml, "times"))
				times = hxml.times;
			if (Reflect.hasField(hxml, "verbose"))
				verbose = hxml.verbose;
			if (Reflect.hasField(hxml, "wait"))
				wait = hxml.wait;
			if (Reflect.hasField(hxml, "xml"))
				xml = hxml.xml;
	...
	**/
	...
	{
		// return Sys.command("haxe " + this.join(" "));
		throw "Don't use that here...";
	}
	...
	@:from public static function fromString(hxml:String):HXML
	{
		...
		if (hxml != null)
		{
			...
			for (line in lines)
			{
				...
				if (str.length == 0 || StringTools.startsWith(str, "#"))
					continue;
				value.push(str);
			}
		}
		...
		for (line in this)
		{
			if (StringTools.startsWith(line, prefix))
			{
				...
				if (makeAbsolute)
				{
					if (cwd != null)
					{
						path = combine(combine(workingDirectory, cwd), path);
					}
					else
					{
...
private function combine(firstPath:String, secondPath:String):String
{
